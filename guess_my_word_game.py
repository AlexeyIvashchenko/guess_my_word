...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "Онлайн крокодил | Игрок 2"))
...
class Window1(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def buttonClicked_2(self):
        w = QWidget()
        self.label = QtWidgets.QLabel("Игрок 1 вводит слово\nОжидайте...", w)
        self.label.setFont(QtGui.QFont("Arial", 14))
    ...
    def send_word(self):
        ...
        main_socket.send(f"{self.text.toPlainText()}_{self.text2.toPlainText()}".encode())
        w = QWidget()
        self.label = QtWidgets.QLabel("Игрок 1 отгадывает слово\nОжидайте...", w)
        self.label.setFont(QtGui.QFont("Arial", 14))
        self.label.move(100, 150)
        self.setCentralWidget(w)
        self.timer = QtCore.QTimer()
    ...
    def wait_msg2(self):
        try:
            ...
            if data:
                print(f"Получены данные: {data}")
                self.data1 = data.split("_")
                self.points2 = int(self.data1[2])
                self.points = int(self.data1[0])
                self.timer.stop()
                if self.data1[0] == "123456":
                        self.win2()
                if self.data1[0]=="654321":
    ...
    def player2(self):
        ...
        self.text.setGeometry(QtCore.QRect(100, 200, 571, 60))
        self.btn = QPushButton("Проверить",self.w)
        self.btn.setGeometry(300,280,100,60)
        self.ltr = 1
    ...
    def checkans(self):
        if self.text.toPlainText().lower() != self.data1[0].lower():
            ...
            if 5 - self.ltr == 0:
                ...
                main_socket.send(f"{self.points2}_Ваше слово не отгадали_{self.points}".encode())
        else:
            self.points2 -= 1
            self.points += 2
            self.label.setText(self.data1[0])
            self.label2.setText("Слово отгаданно!")
            main_socket.send(f"{self.points2}_Ваше слово отгадали_{self.points}".encode())
            QtCore.QTimer.singleShot(3000, self.buttonClicked)
    ...
    def win1(self):
        main_socket.send(f"{"654321"}_Ваше слово отгадали_{self.points}".encode())
        msg_box = QMessageBox()
        ...
        if msg_box.exec() == QMessageBox.Ok:
            exit()
    def win2(self):
        main_socket.send(f"{"123456"}_Ваше слово отгадали_{self.points}".encode())
        msg_box = QMessageBox()
